...
从 2026-08-05 起，每个问题同时记录相邻工作核查状态。`NeedsPriorArtAudit`
表示它只能作为待验证问题，不能突出为研究空白；`Audited` 需要给出检索日期、
范围、三路检索式、最接近工作和覆盖判断。完整协议见
[每日选文与研究缺口核查协议](../docs/daily-selection-and-gap-audit.md)。

## Q001 — 长时序记忆在传感器恢复后会不会继续保留错误？

- **状态**：Active；目前只有一篇锚点论文，需要更多独立证据。
- **相邻工作核查状态**：NeedsPriorArtAudit；截至 2026-08-05，尚未按新协议
  完成三路检索和最接近工作全文核对。
- **公开表述边界**：目前只称“待验证问题”，不得称“学界无人做过”或确定空白。
