...
- `V_t`：本帧相机产生的当前 3D feature；
- `H_t`：从场景 memory 中读取的历史 feature；
- `V_tilde_t`：当前与历史融合后的 feature；
- `M_t`：在 scene coordinate 中持续存在的 memory。
...
iPad App，正文使用白底高分辨率 PNG；每张图都有中文读法和可复制的
[统一 TeX 源文件](../../assets/notes/2026-07-24-st-occ/formulas/source.tex)。
这些 PNG 是按论文原符号重新排版，不冒充论文截图，也不改变代数含义。
...
![公式图：ST-Occ Eq. 5，当前特征与按位姿读取的场景记忆进行融合](../../assets/notes/2026-07-24-st-occ/formulas/eq-05-unified-memory-read.png)
...
**纯文字读法：** `F_tilde_unified^t = psi(F^t | chi(M_t, T_t))`。在时刻
`t`，用车辆 pose `T_t` 从 memory `M_t` 中取出当前位置的历史，再与当前
feature `F^t` 通过融合函数 `psi` 合成新表示。

符号逐个看：

- `F^t`：当前帧新观察产生的 Occupancy feature；
- `M_t`：此前积累的场景 memory；
- `T_t`：当前 ego pose；
- `chi(.)`：从 memory 采样或向 memory 写回的算子；
- `psi`：时序融合函数；
- `F_tilde_unified^t`：融合后的当前 feature。
...
![公式图：ST-Occ Eq. 6，把融合特征写回下一时刻场景记忆的当前区域](../../assets/notes/2026-07-24-st-occ/formulas/eq-06-unified-memory-write.png)

> **公式来源：** Leng et al., ICCV 2025, Eq. (6)，PDF p. 4 /
> proceedings p. 26572；本图按原符号重排。[官方 PDF](https://openaccess.thecvf.com/content/ICCV2025/papers/Leng_Occupancy_Learning_with_Spatiotemporal_Memory_ICCV_2025_paper.pdf) ·
> [可复制 TeX](../../assets/notes/2026-07-24-st-occ/formulas/source.tex#L11-L15)。

**纯文字读法：** `chi(M_(t+1), T_t) = F_tilde_unified^t`。把刚得到的融合
feature 写进下一时刻 memory 的当前 RoI。
...
先看 voxel `p` 的 temporal attributes 定义。

**原文未编号公式：** 论文 Eq. (7) 前的定义，PDF p. 4 /
proceedings p. 26572。

![公式图：ST-Occ 未编号定义，一个 voxel 的时序属性由类别、不确定性和 flow 组成](../../assets/notes/2026-07-24-st-occ/formulas/eq-unnumbered-temporal-attributes.png)
...
**纯文字读法：** `mu_p = {c_p, delta_p, f_p}`。voxel `p` 的时序属性由
类别状态 `c_p`、不确定性 `delta_p` 和 Occupancy flow `f_p` 构成。
...
![公式图：ST-Occ Eq. 7，当前类别激活与历史类别状态递推融合](../../assets/notes/2026-07-24-st-occ/formulas/eq-07-class-activation-update.png)

> **公式来源：** Leng et al., ICCV 2025, Eq. (7)，PDF p. 4 /
> proceedings p. 26572；本图按原符号重排。[官方 PDF](https://openaccess.thecvf.com/content/ICCV2025/papers/Leng_Occupancy_Learning_with_Spatiotemporal_Memory_ICCV_2025_paper.pdf) ·
> [可复制 TeX](../../assets/notes/2026-07-24-st-occ/formulas/source.tex#L21-L32)。

**纯文字读法：** 下一时刻 memory 的类别状态，等于“`alpha × 当前类别激活`
+ `(1-alpha) × 同位置历史类别状态`”经过 softmax。

论文实现设置 `alpha = 0.5`。这意味着新旧 activation 在进入 softmax 前
...
![公式图：ST-Occ Eq. 8，把当前不确定性与 Occupancy flow 写入场景记忆](../../assets/notes/2026-07-24-st-occ/formulas/eq-08-uncertainty-flow-write.png)

> **公式来源：** Leng et al., ICCV 2025, Eq. (8)，PDF p. 4 /
> proceedings p. 26572；本图按原符号重排。[官方 PDF](https://openaccess.thecvf.com/content/ICCV2025/papers/Leng_Occupancy_Learning_with_Spatiotemporal_Memory_ICCV_2025_paper.pdf) ·
> [可复制 TeX](../../assets/notes/2026-07-24-st-occ/formulas/source.tex#L34-L40)。

**纯文字读法：** 下一 memory 的 `delta` 槽写入当前 `delta_t`，`flow` 槽写入
当前 `f_t`。
...
![公式图：ST-Occ Eq. 9，把融合后的 Occupancy representation 写回场景记忆](../../assets/notes/2026-07-24-st-occ/formulas/eq-09-feature-memory-write.png)

> **公式来源：** Leng et al., ICCV 2025, Eq. (9)，PDF p. 4 /
> proceedings p. 26572；本图按原符号重排。[官方 PDF](https://openaccess.thecvf.com/content/ICCV2025/papers/Leng_Occupancy_Learning_with_Spatiotemporal_Memory_ICCV_2025_paper.pdf) ·
> [可复制 TeX](../../assets/notes/2026-07-24-st-occ/formulas/source.tex#L42-L46)。

**纯文字读法：** 下一 memory 的 `V` 槽写入融合 feature `V_tilde_t`。
...
> [!IMPORTANT]
> **[源码] 论文变量和发布代码变量不能直接画等号。** 论文中的 `delta` 是
...
> `1 - global_au`。所以 `global_au` 不是论文 `delta` 的逐值同义变量。
> 见
...
#### Eq. (10)：历史 `H_t` 就是按 pose 从 memory 读出的表示

**原文公式：** 论文 Eq. (10)，PDF p. 5 / proceedings p. 26573。

![公式图：ST-Occ Eq. 10，当前表示以场景记忆中读取的历史表示为条件进行融合](../../assets/notes/2026-07-24-st-occ/formulas/eq-10-memory-attention.png)
...
**纯文字读法：** `V_tilde_t = psi(V_t | H_t) = psi(V_t | chi(M_t, T_t))`。
历史 `H_t` 不是一份固定上一帧张量，而是从 scene memory 读取的当前 RoI。

#### Eq. (11)：gate 由多种证据共同预测

**原文公式：** 论文 Eq. (11)，PDF p. 5 / proceedings p. 26573。

![公式图：ST-Occ Eq. 11，MLP 根据类别、不确定性和特征相似度预测融合权重](../../assets/notes/2026-07-24-st-occ/formulas/eq-11-uncertainty-gate.png)

> **公式来源：** Leng et al., ICCV 2025, Eq. (11)，PDF p. 5 /
> proceedings p. 26573；本图按原符号重排。[官方 PDF](https://openaccess.thecvf.com/content/ICCV2025/papers/Leng_Occupancy_Learning_with_Spatiotemporal_Memory_ICCV_2025_paper.pdf) ·
> [可复制 TeX](../../assets/notes/2026-07-24-st-occ/formulas/source.tex#L59-L62)。

**纯文字读法：** 融合权重 `u` 由 MLP 根据类别状态 `c`、不确定性 `delta`
和当前—历史 representation 的 cosine similarity `epsilon` 共同预测。
...
![公式图：ST-Occ Eq. 12，当前与历史 deformable-attention 分支按权重融合并由 flow 修正采样位置](../../assets/notes/2026-07-24-st-occ/formulas/eq-12-gated-deformable-attention.png)

> **公式来源：** Leng et al., ICCV 2025, Eq. (12)，PDF p. 5 /
> proceedings p. 26573；本图按原符号重排。[官方 PDF](https://openaccess.thecvf.com/content/ICCV2025/papers/Leng_Occupancy_Learning_with_Spatiotemporal_Memory_ICCV_2025_paper.pdf) ·
> [可复制 TeX](../../assets/notes/2026-07-24-st-occ/formulas/source.tex#L64-L75)。

**纯文字读法：** 输出 = `(1-u) × 当前分支 DA + u × 历史分支 DA`；两条
分支都在 `p + f` 处采样，其中 `f` 是 Occupancy flow。
...
![公式图：ST-Occ Eq. 13，把当前 prediction 与 ground truth 写入各自的场景记忆槽](../../assets/notes/2026-07-24-st-occ/formulas/eq-13-prediction-gt-memory-write.png)

> **公式来源：** Leng et al., ICCV 2025, Eq. (13)，PDF p. 5 /
> proceedings p. 26573；本图按原符号重排。[官方 PDF](https://openaccess.thecvf.com/content/ICCV2025/papers/Leng_Occupancy_Learning_with_Spatiotemporal_Memory_ICCV_2025_paper.pdf) ·
> [可复制 TeX](../../assets/notes/2026-07-24-st-occ/formulas/source.tex#L77-L83)。

**纯文字读法：** 把当前预测 `P_t` 和真值 `G_t` 分别写入下一 memory 的
`P` 槽和 `G` 槽，以便在同一场景位置比较时间变化。

#### Eq. (14)：一帧中的 prediction flicker 比例

**原文公式：** 论文 Eq. (14)，PDF p. 5 / proceedings p. 26573。

![公式图：ST-Occ Eq. 14，统计同一场景位置非 Free prediction 的类别变化比例](../../assets/notes/2026-07-24-st-occ/formulas/eq-14-stcv.png)
...
**纯文字读法：** `STCV_t = 发生上述类别变化的 voxel 数 / 当前非 Free
prediction 数`。值越低，按论文定义发生的 temporal flicker 越少。
...
![公式图：ST-Occ Eq. 15，对序列各帧 STCV 求平均得到 mSTCV](../../assets/notes/2026-07-24-st-occ/formulas/eq-15-mstcv.png)

> **公式来源：** Leng et al., ICCV 2025, Eq. (15)，PDF p. 5 /
> proceedings p. 26573；本图按原符号重排。[官方 PDF](https://openaccess.thecvf.com/content/ICCV2025/papers/Leng_Occupancy_Learning_with_Spatiotemporal_Memory_ICCV_2025_paper.pdf) ·
> [可复制 TeX](../../assets/notes/2026-07-24-st-occ/formulas/source.tex#L102-L107)。

**纯文字读法：** mSTCV 是 `T` 帧 `STCV_t` 的平均值。
...
![公式图：ST-Occ Eq. 16，Occupancy loss 由分类、几何语义缩放与深度损失组成](../../assets/notes/2026-07-24-st-occ/formulas/eq-16-occupancy-loss.png)
...
**纯文字读法：** `L_occ = focal + Lovasz-softmax + geometric scaling
+ semantic scaling + depth loss`。

这些项分别鼓励类别预测、集合重叠、几何占用关系、语义关系和深度估计。
初学者不必先背下每一项；此处最重要的是：`L_occ` 负责“预测什么”，下面的
`L_nll` 负责“允许模型怎样表达不确定”，`L_of` 负责“物体怎样移动”。

#### Eq. (17)：为什么模型不能把每道难题都说成“不确定”

**原文公式：** 论文 Eq. (17)，PDF p. 6 / proceedings p. 26574。

![公式图：ST-Occ Eq. 17，用预测 log variance 对残差加权并同时惩罚过大方差](../../assets/notes/2026-07-24-st-occ/formulas/eq-17-uncertainty-nll.png)
...
1. `||y_i - y_hat_i||^2` 是样本 `i` 的平方残差；
2. `s_i` 是预测的 log variance，`exp(-s_i)` 会给残差项降权；
3. 末尾的 `+ s_i` 又会惩罚无约束地扩大 uncertainty。

**学生类比：** 面对模糊路标，学生可以说“这题我不太确定”，老师会降低这道
错题的惩罚；但如果每道题都说“不确定”，额外的 `s_i` 代价会让总分变差。
...
![公式图：ST-Occ Eq. 18，总损失由 Occupancy、不确定性与 Occupancy flow 损失构成](../../assets/notes/2026-07-24-st-occ/formulas/eq-18-total-loss.png)

> **公式来源：** Leng et al., ICCV 2025, Eq. (18)，PDF p. 6 /
> proceedings p. 26574；本图按原符号重排。[官方 PDF](https://openaccess.thecvf.com/content/ICCV2025/papers/Leng_Occupancy_Learning_with_Spatiotemporal_Memory_ICCV_2025_paper.pdf) ·
> [可复制 TeX](../../assets/notes/2026-07-24-st-occ/formulas/source.tex#L133-L140)。

**纯文字读法：** `L = L_occ + L_nll + L_of`。三部分分别对应 Occupancy
预测、不确定性学习和 Occupancy-flow L1 loss。
...
![公式图：ST-Occ 源码简化结构，对 raw uncertainty 做 softplus 与指数变换得到正确类别 variance](../../assets/notes/2026-07-24-st-occ/formulas/source-code-variance-transform.png)
...
**纯文字读法：** `v[i,y_i] = exp(softplus(r[i,y_i]))`。实际代码先对所有类别通道
做 softplus 与指数变换，再按标签取出正确类别的 variance；随后还会对 `v` 做数值
clamp。公式图只保留与单个有效 voxel 损失直接相关的核心结构。

![公式图：ST-Occ 源码简化结构，正确类别负对数概率按 variance 加权并带 log variance 正则](../../assets/notes/2026-07-24-st-occ/formulas/source-code-uncertainty-loss.png)
...
**纯文字读法：** 代码 loss 对有效 voxel `i` 求均值；数据项是“正确类别
...
1. **Uncertainty 表示。** 论文写平均 log variance `delta`；代码经过
...
![公式图：本笔记估算，当前 100×100×8×80 的 FP32 feature 约占 24.4 MiB](../../assets/notes/2026-07-24-st-occ/formulas/calculation-local-feature-memory.png)
...
20 秒序列中，平均相对空间增长 `Delta_mean = 3.25`。按这些数做教学性粗估：

**[判断] 非原文公式：** 以下只估算 feature-like scene state 的量级。

![公式图：本笔记估算，考虑 101 个状态通道与平均场景增长后约为 131 MiB](../../assets/notes/2026-07-24-st-occ/formulas/calculation-scene-state-memory.png)

> **公式来源：** **[判断] 非论文原式。** 101 channels 与
> `Delta_mean = 3.25` 来自补充材料 PDF p. 1；其余为本笔记换算。
