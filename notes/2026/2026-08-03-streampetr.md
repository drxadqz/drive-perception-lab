...
**先建立画面：** **[笔记解释]** 把每个目标 query 当成一张会跨帧传递的车辆档案卡：旧卡保存“上一帧我认为它是谁、在哪”，当前图像递来新证据，更新器把两者合成新卡。

**变量逐项解释与身份：** **[领域惯用]** 上标 *t*、*t*−1 表示当前与上一时刻，时间语义常见但写法不强制；**[本文定义]** ***F***<sub>obj</sub><sup>*t*</sup> 是当前图像形成的对象特征，***F̃***<sub>obj</sub><sup>*t*−1</sup> 与 ***F̃***<sub>obj</sub><sup>*t*</sup> 是时序增强前后的历史/当前对象特征；**[本文定义]** *φ* 是 propagation Transformer 更新函数。式子没有公开单个 query 的具体 shape，固定实现通道为 256。

**变量变化会怎样：** 旧特征清零时，递推退化为只靠当前对象证据；当前图像证据模糊时，输出可能更依赖旧卡。但 *φ* 是可学习非线性函数，不能仅凭某个标量变大断言输出单调增加。
...
**教学小例子：** **[笔记解释]** 假设旧对象卡片认为前车在 20 米处、当前图像只给到一个模糊新
...
**先建立画面：** **[笔记解释]** 相机装在一辆前进的车上，上一帧的“前方 10 米”不能原样贴到当前坐标纸；这两式先移动坐标纸，再把旧目标中心重新落点。

**变量逐项解释与身份：** **[领域惯用]** *t*、*t*−1 是时间索引，inv 或 −1 上标通常表示矩阵求逆，但具体左右乘约定并不通用；**[本文定义]** ***E***<sub>*t*</sub> 与 ***E***<sub>*t*−1</sub> 是当前/历史自车位姿，***E***<sup>*t*</sup><sub>*t*−1</sub> 是历史自车系到当前自车系的 4×4 齐次变换；**[本文定义]** ***Q***<sub>*p*</sub><sup>*t*−1</sup> 是历史三维中心齐次坐标，***Q̃***<sub>*p*</sub><sup>*t*</sup> 是对齐后的当前坐标。

**变量变化会怎样：** 两帧位姿完全相同，变换退化为单位矩阵，中心不动；平移或旋转增大时，对齐位置按矩阵共同改变。矩阵乘法有耦合，不能把旋转角增大简单等同于某个坐标单调增大。
...
**教学小例子：** **[笔记解释]** 如果自车在两帧间前进 1 米，而一辆静止路边车原先位于自车前方
10 米，忽略旋转时其当前相对距离应约为 9 米。这只是教学数字，真实式子使用齐次
4×4 变换并同时处理旋转和平移；这是教学示例，不是论文实验。
...
**先建立画面：** **[笔记解释]** 运动条件像调音台：相对位姿、速度和时间差先进入两个控制器，一个给每个特征通道生成音量旋钮 *γ*，另一个生成基线偏移 *β*。

**变量逐项解释与身份：** **[领域惯用]** *γ*、*β* 常表示仿射缩放与偏移，***v*** 常表示速度，Δ*t* 常表示时间差，但这些字母不是强制标准；**[本文定义]** ***E***<sup>*t*</sup><sub>*t*−1</sub> 是相对自车位姿，***v*** 是对象二维预测速度，Δ*t* 是历史槽到当前帧的时间差；**[本文定义]** *ξ*<sub>1</sub>、*ξ*<sub>2</sub> 是两个可学习映射，输出 256 维 *γ*、*β*。

**变量变化会怎样：** *γ*=1、*β*=0 时调制保持恒等；*γ* 接近 0 会压平对应归一化通道，*β* 改变其中心。输入运动量变大不保证 *γ* 或 *β* 单调变化，因为 *ξ* 是训练得到的非线性映射。
...
**教学小例子：** **[笔记解释]** 若一个归一化通道值为 0.5，条件网络给出 γ=1.2、β=−0.1，
调制后为 0.5；这是教学示例，不是论文实验，只解释仿射操作，不代表论文 learned weights。
...
**先建立画面：** **[笔记解释]** 上一式造好了调音旋钮，这一式真正把同一组旋钮拧到“位置描述”和“内容描述”两条声道上，让历史卡片以适合当前运动条件的方式被读取。

**变量逐项解释与身份：** **[领域惯用]** LN 是 Layer Normalization（层归一化），*γ*、*β* 是逐通道仿射参数，语义常见而符号可换；**[本文定义]** *ψ*(***Q̃***<sub>*p*</sub><sup>*t*</sup>) 是对齐中心的位置编码，***Q***<sub>*c*</sub><sup>*t*</sup> 是历史内容 query；**[本文定义]** ***Q̃***<sub>pe</sub><sup>*t*</sup> 与 ***Q̃***<sub>*c*</sub><sup>*t*</sup> 是调制后的位置、内容特征，点号表示逐元素乘而非矩阵乘。

**变量变化会怎样：** 某通道 *γ* 增大会放大其归一化响应，*β* 增大会整体上移该通道；LN 先消除原始尺度，因此输入向量整体乘常数不等于输出同比增大。两路共用条件不表示两路数值相同。
...
**教学小例子：** **[笔记解释]** 两张内容向量尺度不同的对象卡片经 LN 后都落在可比较范围；若其中
一张来自更久以前，条件网络可以用更大的 Δ*t* 调整某些通道。它不是把“久”必然
映成“低权重”，具体含义由下游检测 loss 学习。这是教学示例，不是论文实验。
