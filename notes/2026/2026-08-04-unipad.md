...
**教学小例子：** [笔记解释] 若两个表面候选权重为 0.7 和 0.3，深度为 8 m 与 12 m，则渲染深度是 9.2 m；颜色也用同样权重混合。例子只解释加权，不代表真实射线只有两个点。
...
**教学小例子：** [笔记解释] 若相邻逻辑值为 0.8 与 0.5，则该段未截断不透明度为 0.375；若此前透射率只有 0.6，最终权重就是 0.225。数值只解释公式。
...
**教学小例子：** [笔记解释] 若平均颜色绝对误差为 0.1、有效射线平均深度误差为 0.2，两个权重都是 10，总损失为 3。若没有有效深度，源码的 mask 会阻断对应深度监督；这个退化条件需要运行时注意。
