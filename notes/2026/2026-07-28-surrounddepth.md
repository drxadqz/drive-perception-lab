...
**先建立画面：** **[笔记解释]** 六台相机各带来一摞压缩后的视觉卡片；每个注意力头像一位编辑，用“当前问题”去检索所有卡片，按相关性摘录内容，最后把八位编辑的摘录拼成新稿。

**变量逐项解释与身份：** **[领域惯用]** *Q*、*K*、*V* 是 query/key/value，Softmax 归一化权重，*d*<sub>*k*</sub> 是每头维数，均属注意力惯用语义但字母并非强制；**[本文定义]** 下标 *i* 是第 *i* 个 head，*O*<sub>*i*</sub><sup>out</sup> 是该 head 输出，*M* 是 head 数且固定源码取 8，*X*<sup>out</sup> 是所有头沿通道拼接的跨视图特征。矩阵转置与乘法方向依论文列/行向量约定解释。

**变量变化会怎样：** 某 query—key 相似度相对其他键增大时，其 Softmax 权重和对应 value 贡献上升；除以 √*d*<sub>*k*</sub> 会在维数增大时压住点积幅度。所有 logits 同加常数不改变 Softmax 权重。
...
**教学小例子：** **[笔记解释]** 只有两个 token 时，若前相机 token 对左前相机 token 的两个
Softmax 权重是 0.2 与 0.8，而值分别是 10 与 20，加权值就是 18。这个数只演示
注意力平均；这是教学示例，不是论文实验或论文特征值。
...
**先建立画面：** **[笔记解释]** 从相机 *i* 的一个像素射出一根光线，用预测深度把它钉在三维空间；再经过车体坐标“转车”，看这枚三维钉子会落到相机 *j* 的哪个像素。

**变量逐项解释与身份：** **[领域惯用]** *K* 常表示相机内参、*T* 常表示齐次变换、*D* 常表示深度、*p* 表示像素/点，但具体方向和左右乘约定不统一；**[本文定义]** *p*<sub>*t*</sub><sup>*i*</sup> 是目标帧相机 *i* 的齐次像素，*D*<sub>*t*</sub><sup>*i*</sup> 是其预测深度，*K*<sup>*i/j*</sup> 是两相机内参，*T*<sup>*i/j*</sup> 是相机相对车体外参；*p*<sub>*t*</sub><sup>*i*→*j*</sup> 是投到相机 *j* 的齐次位置。基线含米制单位，像素坐标没有米单位。

**变量变化会怎样：** 在相机基线固定时，预测深度越大，同一平移造成的视差通常越小；深度越近，视差更明显。旋转、内参和点方向共同耦合，因此不能把深度增大理解为所有像素位移都单调减小。
...
**教学小例子：** **[笔记解释]** 两相机相距 1 m，某点初始预测 1000 m 时，视差近乎为零，投影可能
落不到真实重叠目标；若 SfM 先把它拉到约 10 m，1 m 基线会产生可见视差，空间
光度误差才可能提供有意义监督。这是教学示例，不是论文实验或论文样本。
...
**先建立画面：** **[笔记解释]** 同一辆车只运动一次，但六台朝向不同的相机像六位站在不同方向的观察者；共轭变换把同一物理运动翻译成每位观察者自己的坐标语言。

**变量逐项解释与身份：** **[领域惯用]** *P* 或 *T* 都常用来写刚体变换，逆矩阵表示反向换坐标，但各论文命名与变换方向不同；**[本文定义]** *P*<sub>*t*→*s*</sub> 是目标时刻到源时刻的车体统一 4×4 运动，*T*<sup>*i*</sup> 是第 *i* 台相机到车体的外参，(*T*<sup>*i*</sup>)<sup>−1</sup> 是反向变换；*P*<sub>*t*→*s*</sub><sup>*i*</sup> 是同一运动在相机 *i* 坐标中的表达。

**变量变化会怎样：** *P* 为单位矩阵时，每台相机结果也都是单位矩阵；换相机外参会改变平移与旋转的坐标分量，却不改变物理运动本身。矩阵不交换，左右乘顺序不能互换。
...
**教学小例子：** **[笔记解释]** 车辆直行 1 m 时，朝前相机主要看到沿光轴平移，朝左相机则主要
看到横向平移；不是车辆发生两次运动，而是同一运动在两个坐标系中的分量不同。这是教学示例，不是论文实验。
