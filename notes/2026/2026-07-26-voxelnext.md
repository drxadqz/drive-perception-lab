...
**先建立画面：** **[笔记解释]** 三层稀疏特征像三张网格大小不同的地图：stride 16/32 的坐标先按 2/4 倍换算回 stride 8 的刻度，才能把三张地图上的非空格摆到同一坐标纸上。

**变量逐项解释与身份：** **[领域惯用]** *F* 常表示特征、*P* 表示位置集合、*p* 是单个位置、∪ 是集合并，语义常见但字母不强制；**[本文定义]** (*F*<sub>4</sub>,*P*<sub>4</sub>)、(*F*<sub>5</sub>,*P*<sub>5</sub>)、(*F*<sub>6</sub>,*P*<sub>6</sub>) 是 stride 8/16/32 阶段的稀疏体素特征与三维整数坐标，*P*′<sub>5/6</sub> 分别把坐标乘 2/4，(*F*<sub>*c*</sub>,*P*<sub>*c*</sub>) 是合并结果；(*x*<sub>*p*</sub>,*y*<sub>*p*</sub>,*z*<sub>*p*</sub>) 是体素索引而非米制坐标。

**变量变化会怎样：** 粗尺度坐标每增加 1，映到 stride 8 后分别跳 2 或 4 格；若漏掉倍率，不同尺度的同一物理位置会错位。集合并会增加候选非空位置，但重合坐标的特征如何处理由后续实现决定。
...
**教学小例子：** **[笔记解释]** 这是教学示例，不是论文实验。若 stride 16 的体素坐标是
...
**先建立画面：** **[笔记解释]** 同一地面格上可能竖着多层非空体素，像一栋楼里有多层住户；高度压缩先把它们投到同一个门牌 (*x*,*y*)，再把各层特征相加成一张平面名片。

**变量逐项解释与身份：** **[领域惯用]** 上横线常表示聚合量，求和表示累积，但具体符号不统一；**[本文定义]** *P̄*<sub>*c*</sub> 是去掉高度 *z* 后的非空平面坐标集合，*p̄* 是其中一个 (*x*,*y*)，{*p*∈*P*<sub>*c*</sub> | (*x*<sub>*p*</sub>,*y*<sub>*p*</sub>)=*p̄*} 是该地面柱内所有三维体素；*f*<sub>*p*</sub> 是其特征，*f̄*<sub>*p̄*</sub> 是求和后的平面特征，*F̄*<sub>*c*</sub> 是全部平面特征集合。

**变量变化会怎样：** 柱内新增一个非空体素会把其特征直接加到 *f̄*<sub>*p̄*</sub>；同一特征重复出现会重复计数。求和不除以体素数，因此活跃高度层更多的柱可能有更大幅值，后续网络需自行适配。
...
**教学小例子：** **[笔记解释]** 这是教学示例，不是论文实验。若同一 (*x*, *y*) 柱中两个高度
