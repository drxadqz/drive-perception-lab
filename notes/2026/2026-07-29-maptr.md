...
**先建立画面：** **[笔记解释]** 预测槽像一排没有姓名的选手，GT 地图元素像有姓名的座位；匈牙利匹配尝试所有可行的座位安排，挑总路程最短的一种，而不是让每个选手各抢最近座位。

**变量逐项解释与身份：** **[领域惯用]** 帽号常表示预测，*i* 是 GT 索引，*arg min* 表示寻找最小目标对应的方案；这些语义常见但符号可换；**[本文定义]** *N* 是固定实例槽数，Π<sub>*N*</sub> 是这些槽的排列集合，*π* 是某个具体排列，*π̂* 是最优排列；*ŷ*<sub>π(i)</sub> 与 *y*<sub>*i*</sub> 是配给第 *i* 个 GT 的预测和真值，𝓛<sub>ins-match</sub> 是成对分类—位置匹配代价。

**变量变化会怎样：** 某配对代价增大，会让包含该配对的排列总成本上升；但 *π̂* 是否改变取决于所有槽位的联合成本。新增一个空槽会扩大排列空间，却不必然改变已有真值的最佳配对。
...
**教学小例子：** **[笔记解释]** 假设有三个预测槽 A、B、C，两个 GT 是人行横道
...
**先建立画面：** **[笔记解释]** 同一条车道线可以从左端写到右端，也可以反向写；这条式子像把多种合法“点名顺序”都拿来和预测折线叠图，只用最贴合的一种监督。

**变量逐项解释与身份：** **[领域惯用]** *j* 是点索引，帽号表示预测，*arg min* 是最优选择，曼哈顿距离是逐坐标绝对差之和；概念常见但记号不统一；**[本文定义]** Γ 是当前地图元素允许的等价点序集合，*γ* 是一个候选顺序，*γ̂* 是距离和最小的顺序；*N*<sub>*v*</sub> 是点数，*v̂*<sub>*j*</sub> 是预测点，*v*<sub>γ(j)</sub> 是该顺序下的 GT 点。

**变量变化会怎样：** 任一对应点的横向或纵向误差增大都会线性增加该候选顺序的总距离；但最小值会在不同 *γ* 间切换，因此输出监督顺序并非连续变化。
...
**教学小例子：** **[笔记解释]** 一条两端可交换的三点折线，预测顺序是
右—中—左。若 GT 正向左—中—右的总距离为 8，而反向右—中—左为 0，就选择
反向作为监督；模型不应因为几何正确但书写方向相反而被罚 8。这是教学示例，不是论文实验。
...
**先建立画面：** **[笔记解释]** 把训练看成三位教练共同打分：一位管“是什么地图元素”，一位管“点放在哪里”，一位管“相邻边朝哪个方向”；总损失是三张罚单加权相加。

**变量逐项解释与身份：** **[领域惯用]** 𝓛 表示损失、*λ* 等希腊字母常作权重，但不存在全领域统一字母；**[本文定义]** 𝓛<sub>cls</sub> 是实例 Focal Loss，𝓛<sub>p2p</sub> 是最佳顺序下逐点 L1 位置损失，𝓛<sub>dir</sub> 是相邻边向量的余弦方向损失；*λ*=2、*α*=5、*β*=0.005 是本文三项无量纲权重，𝓛 是总训练目标。

**变量变化会怎样：** 在未加权损失固定时，任一权重增大都会线性提高对应分支对总损失的贡献；但不同损失数值尺度和梯度方向不同，所以小 *β* 不等于方向监督没有影响。
...
**教学小例子：** **[笔记解释]** 若三项未经加权的损失分别为 0.4、0.1、2，
加权后是 0.8、0.5、0.01，总计 1.31。它说明小 *β* 不等于方向分支没有作用；
各损失的数值尺度不同。这是教学示例，不是论文实验。
...
**先建立画面：** **[笔记解释]** 像给地图上的每个点贴两张标签：一张写“我属于第几个实例”，另一张写“我是该实例的第几个点”；两张 embedding 相加后形成唯一的组合身份。

**变量逐项解释与身份：** **[领域惯用]** *q* 常表示 query，*i*、*j* 常作实例与点索引，但字母并非统一；**[本文定义]** *q*<sub>*i*</sub><sup>ins</sup> 是第 *i* 个地图实例 query，*q*<sub>*j*</sub><sup>pt</sup> 是所有实例共享的第 *j* 个点位 query，*q*<sub>ij</sub><sup>hie</sup> 是二者逐元素相加后送入 decoder 的层次 query；三者在固定实现中以 256 维 content/position 分量展开。

**变量变化会怎样：** 改变实例 query 会同时影响该实例的全部点，改变点位 query 会同时影响所有实例的同序号点；两者相加后无法从结果唯一反解各自贡献，这正是共享参数带来的耦合。
...
**教学小例子：** **[笔记解释]** 两个实例 query 加三个点 query，会得到六个组合；
点 1 query 在两个实例间共享“这是第一个点”的角色，实例 A query 则在三个点间
共享“都属于实例 A”的角色。这是教学示例，不是论文实验。
