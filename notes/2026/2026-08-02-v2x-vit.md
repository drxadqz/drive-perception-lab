...
<p align="center"><picture><source media="(prefers-color-scheme: dark)" srcset="../../assets/notes/2026-08-02-v2x-vit/formulas/eq-01-hmsa-aggregation-dark.png"><img src="../../assets/notes/2026-08-02-v2x-vit/formulas/eq-01-hmsa-aggregation-light.png" alt="公式：目标节点在邻居集合中按注意力权重汇总类型化消息，再做目标类型专属投影" width="478" height="140"></picture></p>

> **公式来源：** Xu et al., ECCV 2022, Eq. (1)，PDF p. 7；本图按原符号重排。
> [官方 PDF](https://www.ecva.net/papers/eccv_2022/papers_ECCV/papers/136990106.pdf) ·
> [可复制 TeX](../../assets/notes/2026-08-02-v2x-vit/formulas/source.tex#L5-L15)。

**先建立画面：** **[笔记解释]** 把 ego、邻车和路侧单元想成围着同一张 BEV 方格地图开会：每位邻居递来一张消息卡，注意力决定这张卡占多少票，目标节点再按自己的类型汇总会议纪要。

**变量逐项解释与身份：** **[领域惯用]** *i* 常作接收节点、*j* 作发送邻居，𝒩(*i*) 作邻居集合，求和是消息聚合；这些角色在图网络中常见但字母不强制；**[本文定义]** ***H***<sub>*i*</sub> 是目标节点更新后的同位置 BEV 特征，*c*<sub>*i*</sub> 是车辆/基础设施类型；ATT(*i*,*j*) 是归一化权重，MSG(*i*,*j*) 是按来源与有向边类型变换的消息，Dense<sub>*cᵢ*</sub> 是目标类型专属投影。

**变量变化会怎样：** 某邻居 ATT 权重增大时，其 MSG 对聚合向量的直接占比增大；MSG 为零则该邻居即使权重大也不贡献内容。softmax 使权重彼此竞争，因此提高一个邻居通常会压低其他邻居占比。
...
**教学小例子：** **[笔记解释]** 同一 BEV cell 有 ego、另一辆车和路侧节点三份证据，注意力为
...
> [官方 PDF](https://www.ecva.net/papers/eccv_2022/papers_ECCV/papers/136990106.pdf) ·
> [可复制 TeX](../../assets/notes/2026-08-02-v2x-vit/formulas/source.tex#L17-L27)。

**先建立画面：** **[笔记解释]** 同一句“这里有车”，从路侧发给车辆与从车辆发给路侧可能含义不同；关系矩阵像按“谁对谁说”更换的翻译词典，然后 key 与 query 才比较是否匹配。

**变量逐项解释与身份：** **[领域惯用]** ***K***、***Q*** 表示 key/query，*m* 表示注意力头，*i*、*j* 表示接收/发送节点，点积除平方根是常见缩放注意力语义，但字母和方向可换；**[本文定义]** *e*<sub>ij</sub> 是有向边，*φ* 把它映成 V→V、V→I、I→V 或 I→I 类型，***W***<sub>φ(eᵢⱼ)</sub><sup>*m*,ATT</sup> 是该关系和 head 专属矩阵，*C* 是论文缩放通道记号，head<sub>ATT</sub><sup>*m*</sup> 是未归一化分数。

**变量变化会怎样：** key 与经关系矩阵变换后的 query 越同向，点积分数越大；*C* 增大使除数变大、分数幅度收缩。但 ***W*** 同时混合维度，不能从某个原始通道变大直接推出总分必增。
...
**教学小例子：** **[笔记解释]** 同一个路侧 key 分别发给车辆和另一个路侧节点时，会选 I→V
或 I→I 两套不同矩阵，因此即使原始 key 相同，匹配分数也可不同。这是教学示例，不是论文实验。
...
> [官方 PDF](https://www.ecva.net/papers/eccv_2022/papers_ECCV/papers/136990106.pdf) ·
> [可复制 TeX](../../assets/notes/2026-08-02-v2x-vit/formulas/source.tex#L29-L39)。

**先建立画面：** **[笔记解释]** 延迟不是只贴一个“旧”标签，而是像把同一时间差送进一排快慢不同的钟摆；各通道产生不同相位，后续网络据此辨认证据有多旧。

**变量逐项解释与身份：** **[领域惯用]** Δ*t* 常表示时间差，sin/cos 是正余弦，偶/奇通道配对来自 Transformer 位置编码惯例，但具体字母和常数并非统一标准；**[本文定义]** Δ*t*<sub>*i*</sub> 是节点 *i* 的通信/观测延迟，*p*<sub>*c*</sub> 是第 *c* 个编码值，*C* 是总通道数，*k* 是满足 *c*=2*k* 或 2*k*+1 的整数索引，10000 是本文沿用的频率基数。

**变量变化会怎样：** 延迟增大时每个通道在 −1 到 1 间周期变化，不是单调变大；低频通道变化慢，高频通道变化快。仅看单个通道会有周期歧义，完整向量才提供多尺度时间指纹。
...
**教学小例子：** **[笔记解释]** 100 ms 和 400 ms 会得到不同 256 维相位向量；线性层可据此
学习“越旧的邻居证据应怎样调整”。这是教学示例，不是论文实验；编码不会自动把移动目标推到未来位置。
...
> [官方 PDF](https://www.ecva.net/papers/eccv_2022/papers_ECCV/papers/136990106.pdf) ·
> [可复制 TeX](../../assets/notes/2026-08-02-v2x-vit/formulas/source.tex#L41-L48)。

**先建立画面：** **[笔记解释]** 上一式把延迟写成一张 256 维“时间身份证”，这里先用线性层翻译成特征语言，再把同一身份证贴到该节点整张 BEV 地图的每个格上。

**变量逐项解释与身份：** **[领域惯用]** *f* 常表示函数、加号表示残差式相加，但字母不统一；**[本文定义]** ***p***(Δ*t*<sub>*i*</sub>) 是节点 *i* 的完整延迟编码，*f* 是可学习线性投影，DPE(Δ*t*<sub>*i*</sub>) 是投影后的 256 维条件向量，***H***<sub>*i*</sub> 是该节点 176×48×256 的 BEV 特征；Eq. (10) 的加法沿空间维广播。

**变量变化会怎样：** DPE 为零时特征不变；某通道 DPE 增大时，该节点所有空间格的对应通道都等量上移。空间位置本身没有移动，所以延迟增大不能由这条式子单独推出目标被补偿到未来位置。
...
**教学小例子：** **[笔记解释]** 对 176×48 个 cells，同一节点都加相同 256 维延迟向量；空间位置
不变，变化的是通道条件。这是教学示例，不是论文实验。
